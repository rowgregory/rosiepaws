...
import { getMovementsByType, getAverageActivity } from '@/app/lib/utils/public/my-pets/movements/statUtils'
...
import LatestMovement from '@/app/components/guardian/movements/LatestMovement'

const MovementTracking = () => {
  const { zeroMovements, movements } = useAppSelector((state: RootState) => state.movement)
  const todaysMovements = getTodaysMovements(movements || [])
  const weeklyStats = getWeeklyMovementStats(movements || [])

  const shouldAnimate = useInitialAnimation(movements)
          ...
          <div className="xl:col-span-3 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {movements?.map((movement: IMovement, index: number) => (
                <MovementCard key={movement.id} movement={movement} index={index} shouldAnimate={shouldAnimate} />
              ))}
            </div>
          </div>
