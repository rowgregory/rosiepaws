  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const updatedUser = await tx.user.update({
        where: { id: user.id },
        data: {
          ...(!user.isLegacyUser && { tokens: { decrement: painScoreUpdateTokenCost } }),
      ...
      await tx.tokenTransaction.create({
        data: {
          ...
          type: user.isLegacyUser ? 'PAIN_SCORE_UPDATE_LEGACY' : 'PAIN_SCORE_UPDATE',
          description: `Pain score update${user.isLegacyUser ? ' (Usage Tracking Only)' : ''}`,
          metadata: {
